 ...
 */
module.exports = async ({cwd, env, name, lastRelease: {gitHead: from}, nextRelease: {gitHead: to = 'HEAD'} = {}, logger}, path) => {
  const debug = getDebug(name, 'get-commits');

  path && debug('Get commits in path: %o', path);
  ...
  const commits = await getCommits(from, to, {cwd, env}, path);
