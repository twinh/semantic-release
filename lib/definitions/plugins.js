...
const {isString, isPlainObject, template} = require('lodash');
const semver = require('semver');
const {getGitHead} = require('../git');
const hideSensitive = require('../hide-sensitive');
const {hideSensitiveValues} = require('../utils');
const {RELEASE_TYPE, RELEASE_NOTES_SEPARATOR, FIRST_RELEASE} = require('./constants');
...
module.exports = {
  ...
  analyzeCommitsAll: {
    ...
    postprocess: (results) => {
      return results.filter(Boolean).reduce((nextReleases, result) => {
        Object.entries(result).forEach(([name, values = {}]) => {
          const {nextReleaseType, nextReleaseVersion} = values;

          if (!nextReleaseType && !nextReleaseVersion) {
            return;
          }

          if (typeof nextReleases[name] === 'undefined') {
            nextReleases[name] = {};
          }

          if (
            nextReleaseType &&
            RELEASE_TYPE.indexOf(nextReleaseType) > RELEASE_TYPE.indexOf(nextReleases[name].nextReleaseType)
          ) {
            nextReleases[name].nextReleaseType = nextReleaseType;
          }

          if (
            nextReleaseVersion &&
            semver.gt(nextReleaseVersion, nextReleases[name].nextReleaseVersion || FIRST_RELEASE)
          ) {
            nextReleases[name].nextReleaseVersion = nextReleaseVersion;
          }
        });
        return nextReleases;
      }, {});
  ...
  generateNotes: {
    ...
    postprocess: (results, {env, options, nextRelease: {version}}) => {
      if (options.versionMode === 'fixed') {
        results.push(
          [
            options.fixedReleaseTitle || '### Release',
            '',
            template(options.fixedReleaseContent || `* upgrade all packages to \${version}`)({version}),
          ].join('\n')
        );
      }

      return hideSensitive(env)(results.filter(Boolean).join(RELEASE_NOTES_SEPARATOR));
    },
  },
  generateNotesAll: {
    required: false,
