...
module.exports = (context, type, pluginOpt, pluginsPath) => {
  ...
  const validator = async (input) => {
    // Prefer to input logger, which have custom scopes with package name
    const {logger = defaultLogger} = input;
    ...
    try {
      if (!input.options.dryRun || dryRun) {
        ...
        let scopes = [];
        if (Array.isArray(logger.scopeName)) {
          scopes = logger.scopeName.concat(pluginName);
        } else {
          scopes = [logger.scopeName, pluginName];
        }
