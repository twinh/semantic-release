...
module.exports = async (context, cliOptions) => {
  ...
  const monorepo = options.packages || packageJson.workspaces;
  if (monorepo && !options.tagFormat) {
  ...
  options = {
    ...
    monorepo,
    packages: await pkgPackages(packageJson),
...
async function pkgPackages(packageJson) {
  return packageJson.workspaces ? packageJson.workspaces : ['.'];
}
