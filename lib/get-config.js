...
module.exports = async (context, cliOptions) => {
  ...
  const {packageJson} = (await readPkgUp({normalize: false, cwd})) || {packageJson: {}};
  options = {
    branches: [
      '+([0-9])?(.{+([0-9]),x}).x',
      'master',
    ...
    repositoryUrl: (await pkgRepoUrl(packageJson)) || (await repoUrl({cwd, env})),
    tagFormat: `v\${version}`,
    plugins: [
    ...
    ...getMonorepoOptions(options, packageJson),
...
function getMonorepoOptions(options, packageJson) {
  let packages = [];
  let monorepo = false;
  let tagFormat;
  ...
  if (monorepo && options.versionMode !== 'fixed') {
    tagFormat = `\${name}@\${version}`;
  } else {
    tagFormat = `v\${version}`;
  }

  return {tagFormat, packages, monorepo};
}

async function pkgRepoUrl(packageJson) {
