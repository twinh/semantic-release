...
const {MIN_RELEASE, RELEASE_TYPE} = require('../definitions/constants');
const {
  tagsToVersions,
  isMajorRange,
  getUpperBound,
...
function maintenance({maintenance, release}) {
  return sortBy(
    ...
    const min = highest(getLatestVersion(versions) || MIN_RELEASE, maintenanceMin);
...
function release({release}) {
  ...
  // The initial lastVersion is the last release from the base branch of `MIN_RELEASE`
  let lastVersion = getLatestVersion(tagsToVersions(release[0].tags)) || MIN_RELEASE;
